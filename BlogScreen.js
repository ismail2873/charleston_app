   ...
   <Image source={{ uri: 'asset:/images/logo_dark.png' }}
    ...
    <View >
     ...
     {blog_posts.map((item, index)=>{
        ...
        return(
         <View  key={item.id || index} style={{ margin: 20 ,marginTop:10,borderColor:'#c8c5c5ff',borderWidth:2,borderRadius:22,backgroundColor:'#ffffffff'}}>
   
   
           <View style={{ flex: 1, width: '100%', height: '300' ,}}>
             <ImageBackground
               source={{uri:"https://charlestonrestaurantguide.com/assets/backend/media/blog_uploads/"+item.image}}
           ...
           <Text style={{paddingHorizontal:10, color: 'rgb(98, 97, 97)', marginTop: 10, fontSize: 18, fontWeight: 'bold' }}>{item.author} {item.updated_at}</Text>
           <Text style={{paddingHorizontal:10, fontSize: 22, fontWeight: '500',marginVertical: 10 }}>{item.title}</Text>
          <Text style={{paddingHorizontal:10,fontSize:16}} numberOfLines={expandedItems[item.id] ? undefined : 2}>
  {item.content}
</Text>
<TouchableOpacity onPress={() => toggleText(item.id)}>
  <Text style={{ color: 'blue', marginTop: 5, paddingHorizontal: 10 }}>
    {expandedItems[item.id] ? 'Show less' : 'Show more'}
  </Text>
</TouchableOpacity>
           <Text style={{ color: '#999', fontSize: 18, fontWeight: 'bold', marginTop: 20, borderTopColor: 'lightgrey', borderTopWidth: 1, padding: 20 }}>{item.author}</Text>
...
<View key={item.id || index} style={{ padding: 20 }}>
           ...
           <View style={{flexDirection:'row',width:'100%',marginVertical:20}}>
             <View style={{width:'30%'}}>
               <Image  source={{uri:"https://charlestonrestaurantguide.com/assets/backend/media/blog_uploads/"+item.image}}
               
               
                 style={{ width: '100', height: '140' }} />
             </View>
             <View style={{width:'70%'}}>
               <Text style={{fontWeight:'bold',color:'rgb(109, 106, 106)'}}>{item.updated_at}</Text>
